...
mod utils;
use utils::{
    approve_template, assert_attr_eq, assert_template_not_exists, assert_template_state,
    create_test_template_msg, instantiate_contract, publish_template, reject_template,
};
...
fn test_publish_template_and_reject() {
    ...
    assert_attr_eq(&response.attributes[0], "method", "request_for_approval");
    assert_attr_eq(&response.attributes[1], "template_id", "template1");
    assert_attr_eq(
    ...
    assert_attr_eq(&reject_response.attributes[0], "method", "reject_template");
    assert_attr_eq(&reject_response.attributes[1], "template_id", "template1");
    assert_attr_eq(
...
fn test_publish_template_and_approve() {
    ...
    assert_eq!(response.attributes.len(), 3);
    assert_attr_eq(&response.attributes[0], "method", "request_for_approval");
    assert_attr_eq(&response.attributes[1], "template_id", "template2");
    assert_attr_eq(
        &response.attributes[2],
        "publisher",
        &publisher_address.to_string(),
    ...
    assert_attr_eq(&approve_response.attributes[1], "template_id", "template2");
    assert_attr_eq(
