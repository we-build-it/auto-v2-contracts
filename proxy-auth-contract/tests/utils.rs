use cosmwasm_std::{testing::message_info, Addr, DepsMut, Env, Response};
use std::collections::HashSet;

use proxy_auth::{
    contract::{execute, instantiate},
    msg::{ActionMsg, ExecuteMsg, InstantiateMsg, TemplateMsg},
    state::load_template,
};

pub fn assert_attr_eq(attr: &cosmwasm_std::Attribute, expected_key: &str, expected_value: &str) {
    assert_eq!(
        attr.key,
        expected_key,
...
pub fn instantiate_contract(deps: DepsMut, env: Env, admin_user_address: Addr) -> Addr {
...
pub fn create_test_template_msg(template_id: &str, publisher: Addr, private: bool) -> TemplateMsg {
    TemplateMsg {
        ...
        actions: vec![
            ActionMsg {
                ...
                message_template:
                    "{\"swap\": {\"amount\": \"{{amount}}\", \"denom\": \"{{denom}}\"}}".to_string(),
...
) -> Result<Response, proxy_auth::error::ContractError> {
    let msg_info = message_info(&publisher, &[]);
    let msg = ExecuteMsg::RequestForApproval {
